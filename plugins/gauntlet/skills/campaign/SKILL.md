...
| `nudge.py` | Advisory reminder printer for heartbeat start: reminders computed from the ledger, then the hand-written standing notes in `.gauntlet/nudges.md` (located as `--followups`'s sibling) appended through its shared safe encoder; always exits 0 | its own module docstring |
