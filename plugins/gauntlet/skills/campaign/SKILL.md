...
| `clean-rebase.py` | `run` — EXECUTE the clean base-only rebase (fetch/rebase/force-with-lease push + the ledger write that **carries `reviews_ok` and the status label forward** on a shape-preserving rebase, setting `ci = pending`, resetting the liveness counters, and — because the head moves — voiding the base-preflight stamp `base_ok_sha` at the accessor door, so a fresh `proceed` is re-earned before the next verdict) and REFUSE everything else: a conflict or a diff-changing rebase is aborted/reset and handed back (exit 3), never resolved | `references/stage-2-review-gate.md` |
