...
- **Branch ownership is data:** read `branch_ownership` from ledger every wake — default `declined`. It governs **only** the adopted PR's **remote** head branch: campaign deletes that remote branch on merge only when ownership is `granted` (from an explicit user instruction in the invocation — natural language, like naming the reviewer, NOT a CLI flag — or a stored preference), resolved once at run start by the same explicit > preference > default shape as `reviewer`. **Local cleanup is never governed by `branch_ownership`:** the worktree and local branch are removed only per the per-PR `worktree_owned`/`branch_owned` flags in **both** modes, and a reused worktree, the root/main checkout, or a reused local branch is **never** removed. An unattended run with no stored grant stays `declined`. Never hold the run on a live prompt for it.
