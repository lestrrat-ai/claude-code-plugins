...
| **Finding-audit worker** (verdicts each gating finding) | **`session`** | Gate-adjacent: its CONFIRMED / ADJUSTED / REFUTED verdict decides **whether and what** gets fixed — only CONFIRMED/ADJUSTED reach the fix subagent (`references/finding-audit.md`). A weaker model mis-verdicts a finding and the wrong thing, or nothing, gets fixed. Never downgraded. |
