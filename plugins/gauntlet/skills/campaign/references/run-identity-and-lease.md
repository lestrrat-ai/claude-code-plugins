...
| scheduled heartbeat prompt | the lean same-session wake prompt `heartbeat.py callback` prints (`runtime-adapter.md`, "Background work and heartbeats", owns the shape): it re-enters the campaign skill already loaded in the live session and carries **only** `--run <run-id> --token <agent-token>` — so a summarized heartbeat re-proves ownership without guessing — embedding `<campaign-invocation>` once as its not-in-context fallback. It **never** carries `--new` or the original `#PR` adoption args: those are **start-time-only** (they *create/adopt*), whereas `--run` **resumes** an existing run — replaying `--new` on a scheduled heartbeat would mint a fresh run every heartbeat. |
