...
| `review-<pr>-<n>.progress.jsonl` | Reviewer progress events against the plan for round `n` (launch attempt 1), opened by the orchestrator's `pass_identity` line. **Every line is READ and validated through `scripts/review-pass.py`, and every reviewer-written line goes through one of its doors.** The orchestrator's `pass_identity` (`review-pass.py identity`), the unit-progress events (the reviewer's `emit-progress.py`), and a `plan_amendment_request` (the reviewer's `emit-amendment.py`, which forwards to `review-pass.py amend`) all do — no event is exempt. `stage-2-review-gate.md` owns that rule; see "Review-pass artifacts" below |
