...
| `review-<pr>-<n>.prompt.txt` | The reviewer prompt for round `n`, launch attempt 1, with the verbatim intent and JSON-encoded typed transport record bound as data. `n` is the pass number, spent only by a landed verdict (`runtime-adapter.md`, "Review preparation mapping"). Written only by `scripts/review-dispatch.py prepare`, then passed through `dispatch_native` or `run_argv` — never embedded in shell source (`review-dispatch.md`) |
