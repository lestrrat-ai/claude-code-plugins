...
| `review-<pr>-<n>.allocation.jsonl` | Durable, append-only driver-owned allocation/recovery journal for pass `n`: `scripts/review-dispatch.py prepare` writes fresh allocations; `result` writes outcomes and can migrate one validated active legacy allocation. `runtime-adapter.md`, **Review allocation journal**, owns its lifecycle. It is not review evidence and `review-pass.py` never reads it |
...
| `codex-home/` | The `external-codex` reviewer's codex home, built once per run by `scripts/review-dispatch.py prepare` and passed to the reviewer as `CODEX_HOME`. A directory, not a file, and the only run artifact that is not review evidence about a PR. `review-dispatch.md`, **The reviewer's codex home**, owns what it contains and why; `review-dispatch.py` owns the exact contents. It holds symlinks into the operator's real codex home, so **NEVER copy, archive, or publish it** — following its links reaches live credentials |
...
| `.gauntlet/nudges.md` | Durable, and **not run-scoped** — one file, shared by every run. The **standing-notes file**: lessons the campaign has learned about its own PROCESS, which no rule in code can hold, delivered to a fresh heartbeat that would otherwise have to remember them. **Hand-written by the driver or the user** — nothing in the plugin writes it, and **no file of that name IN THIS DIRECTORY** is the normal, silent case. A `.gauntlet/` that is not there at all is not that silence: nothing was looked at, and it is disclosed. `scripts/nudge.py` READS it each heartbeat and appends each usable line, through nudge's shared safe encoder, after its computed reminders; it locates the file as **`--followups`'s sibling in this directory**, which is why that path is derived rather than a flag of its own (`loop-control.md`, Step 1). Its content model, its disclosed caps, and every case it DISCLOSES as not read rather than passing off as that silence are owned by `nudge.py` (`read_notes`). |
...
- `repair_decision` — `-`, or the last reassessment decision + when: `<decision>@<iso>`. Durable, because
  ...
  **DURABLE *and* SPENT EXACTLY ONCE per cap** — it is reset to `-` when the row **re-enters `repairing`**
  ...
  `abort@…` is terminal and is never cleared. Existing `demote@…` values are legacy decisions: keep them
  readable and dispatchable without migration; `repair-pass.md`, "Complete a legacy DEMOTE", owns their
  completion.
