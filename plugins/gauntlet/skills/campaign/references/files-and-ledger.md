...
| `failure-<pr>-<n>-<k>.txt` | The captured output of the FAILED external review attempt `k` of pass `n` — the `failure_capture` the review transition reads before it decides whether the one retry waits. Written by the driver through the runtime adapter's `write_bytes`, one file per attempt, never over another attempt's file and never over the attempt's report. `runtime-adapter.md`, "Review isolation capability and transition", owns what it must contain |
