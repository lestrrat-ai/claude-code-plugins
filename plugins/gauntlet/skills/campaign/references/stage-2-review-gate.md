...
- **HUMAN-DOC** — human-facing prose only: top-level `README.md`, human prose under `docs/**`,
  `CHANGELOG`, `LICENSE`. Every otherwise eligible path is checked for frontmatter first.
- **CODE** — source files **and agent-consumed docs**: `SKILL.md`, a skill's `references/**`,
  ...
  skill/agent frontmatter. Frontmatter the mechanical extractor cannot fully parse, including escaped
...
- **Base currency with `<base>`.** Throughout Stage 2, `<base>` is **this PR row's effective base** — its
  ...
  sets `ci = pending`, and the tool writes the new head through the accessor, which fires the head-move
...
| `unusable` | 1 | the artifacts are **defective** — the active report is missing, empty, not exactly ONE `review_report` record, or malformed in that record (writing no residual-risk record never makes a report defective, and neither does writing several); a short SHA or other malformed identifier; invalid progress/identity/findings; **no usable intent block**; a bound `default_non_goals` that no longer matches the run's live defaults (scope drift); a parsed result that does not cohere with findings; or a spurious DEFERRED result | the pass **CANNOT count**. Fix skipped adoption inputs when named; otherwise retry — the same pass, next launch attempt (`runtime-adapter.md`, "Review preparation mapping") — on the route the transition returns: defective artifacts say nothing about the engine, so they are a `transient` failure that relaunches the SAME reviewer until its ordinary allocations run out (`reviewer.md`, "External failure classes") |
...
`ci = pending` — writing the new head through the accessor fires the head-move reset at the door
(`files-and-ledger.md`, the `head_sha` field, "What a genuine head move resets"). The moment PR content changes — review fix, CI fix, a judgment-path rebase (conflict-resolving or diff-changed), a
...
writing the new head through the accessor fires the head-move reset at the door (`files-and-ledger.md`,
the `head_sha` field, "What a genuine head move resets") — the gate and the counters key off **different**
events, and this row is exactly where they part company.
