...
| `unusable` | 1 | the artifacts are **defective** — the active report is missing, empty, truncated, duplicate, nonterminal, malformed, or carries a misplaced or malformed residual-risk line (its ABSENCE is fine); a short SHA or other malformed identifier; invalid progress/identity/findings; **no usable intent block**; a bound `default_non_goals` that no longer matches the run's live defaults (scope drift); a parsed result that does not cohere with findings; or a spurious DEFERRED result | the pass **CANNOT count**. Fix skipped adoption inputs when named; otherwise retry — the same pass, next launch attempt (`runtime-adapter.md`, "Review preparation mapping") — or take the fresh-worker fallback. For an EXTERNAL pass, take the backoff owner's action first (`reviewer.md`, "Running the cross-engine reviewer"), so a refusal is `stop-and-ask` rather than a retry |
