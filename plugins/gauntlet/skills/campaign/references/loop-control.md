...
1. **Resolve repository context, then the run + lease, then init / resume / start fresh.** Call
   ...
   on the reminders through those owners; ignore none silently. Its output is driver-internal
...
3. **Dispatch due work — non-blocking, idempotent, bounded, work-conserving.** Scan the whole run,
   ...
   **Non-zero does NOT mean "held".** The command is an allow-list on `in_review`, so it also refuses a
   ...
   - **`repairing`** — the PR reached a **review-loop cap** and has stopped converging. It normally is **NOT
     ...
     (`dispatch-check --action repair`). Its non-legacy recorded repair's required clean base-only rebase is
     part of that decision; `repair-pass.md`, **A non-legacy recorded repair may first take its required clean base-only
   ...
   **A `repairing` PR is the ONE held PR that still has machine work due — and it is NOT the work the
   ...
   - **a legacy `demote@…` is recorded** → `ledger.py dispatch-check --pr <N> --action repair`, then
     follow `repair-pass.md`, **"Complete a legacy DEMOTE"**, directly. Do not clear a base precondition
     or run `clean-rebase.py`.
   - **another `repair_decision` is recorded** → `ledger.py dispatch-check --pr <N> --action repair`, then
     clear the decision's base precondition. On `rebase-first`, follow `repair-pass.md`, **A non-legacy
     recorded repair may first take its required clean base-only rebase**; otherwise execute **that**
     decision and no other work. When the repair has landed, return the row to the gate
