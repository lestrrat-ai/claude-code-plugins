...
3. **Dispatch due work — non-blocking, idempotent, bounded, work-conserving.** Scan the whole run,
   ...
   **The test is "does this MUTATE the PR?" — NOT "is this action named in a list?"** *Mutate* = change
   ...
   - **Only the user's answer unparks a PR — and EVERY park class names the durable record it is
     ...
     | **`awaiting-user`, machine blocker** — campaign cannot move this PR without a human; that **property** is the class, and these are only its **current members**: CI SETTLED-not-green, a check stuck `RUNNING` past the CI STALL CAP (`stage-2-ci.md`, "RUNNING-STALL"), an UNUSABLE snapshot at its cap, an unrecognized CI enum value, a `BLOCKED`/unrecognized `mergeStateStatus`, a draft PR (`files-and-ledger.md`, `status`, owns the class; `ci_reason` names the blocker at every one of them) | `blocker_ruling` = `retry@<iso>` / `abort@<iso>` | `retry` → `in_review`, **RESET THE LIVENESS COUNTERS**, and **SPEND the ruling: `blocker_ruling` = `-`** (`stage-2-ci.md`, "THE LIVENESS COUNTERS" / "THE RULING IS CONSUMED EXACTLY ONCE"), then re-derive CI on the next wake; `abort` → terminal `aborted` (the ruling **stays** — it is the record of why) |
