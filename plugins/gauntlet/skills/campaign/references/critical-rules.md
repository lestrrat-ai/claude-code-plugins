...
- **NEVER leave `gauntlet-accepted` on a PR whose live content no longer holds `required(tier)`
  ...
  fix commit, a conflict-resolving rebase, any other content change on the head branch, **or a
  depth-raising tier escalation, which voids the tally on unchanged content**), OR a tier **de-escalation**
  that lowers `required(tier)` on unchanged content — MUST also reconcile the label by running
