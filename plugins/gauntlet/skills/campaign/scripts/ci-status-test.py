...
def command_copy_cases(ci, tmp: Path) -> list[str]:
    ...
    root = tmp / "command-copy-required-set-commonmark-fences"
    root.mkdir()
    (root / "commands.md").write_text(
        "  ~~~sh\n"
    ...
    if len(copies) != 2 or checked != ["commands.md:2", "commands.md:6"]:
        problems.append(
            f"[command copy required-set] CommonMark fences were not both discovered: "
            f"{copies!r}, checked={checked!r}"
        )
    if len(found_problems) != 1 or "commands.md:6" not in found_problems[0]:
        problems.append(
