...
| [`gauntlet:address-followups`](skills/address-followups/SKILL.md) | Work that queue instead of showing it: resume each entry at the step its state has already reached, investigating a still-unverified one with a read-only subagent that must reproduce or refute it, open one PR per entry it may act on, and hand those PRs to a campaign. |
...
(`skills/campaign/references/reviewer.md`, "Selecting the reviewer", owns which sources count). The
pipeline falls back to a fresh native worker only when the cross-engine reviewer is genuinely unusable:
the paired CLI is absent, the engine itself can't produce a verdict (quota or usage limits, auth, a
process that won't run), or retries of a transient failure (a timeout, a bad report, a launch fault) ran
out. A reviewer that refuses to review the content parks the PR for you instead of downgrading the
review. The existing Codex retry uses repository-maintenance
